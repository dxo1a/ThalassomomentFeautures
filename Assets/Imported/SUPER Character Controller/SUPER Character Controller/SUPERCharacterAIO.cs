...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    #region Walking/Sprinting/Crouching
    ...
    public float statTickRate = 6.0f, hungerDepletionRate = 0.06f, hydrationDepletionRate = 0.14f, oxygenDepletionRate = 0.2f;
    ...
    public void TickStats(){
        ...
        if(currentSurvivalStats.Oxygen>0){
            currentSurvivalStats.Oxygen = Mathf.Clamp(currentSurvivalStats.Oxygen-(oxygenDepletionRate+(isSprinting&&!isIdle ? 0.1f:0)), 0, defaultSurvivalStats.Oxygen);
            currentSurvivalStats.isGasping = (currentSurvivalStats.Oxygen<(defaultSurvivalStats.Oxygen/8));
        }
    ...
    public void ImmediateStateChange(float Amount, StatSelector Stat = StatSelector.Health){
        switch (Stat){
            ...
            case StatSelector.Oxygen:{
                currentSurvivalStats.Oxygen = Mathf.Clamp(currentSurvivalStats.Oxygen+Amount,0,defaultSurvivalStats.Oxygen);
                currentSurvivalStats.isGasping = (currentSurvivalStats.Oxygen<(defaultSurvivalStats.Oxygen/8));
            }break;
        }
    }
    public void LevelUpStat(float newMaxStatLevel, StatSelector Stat = StatSelector.Health, bool Refill = true){
        switch(Stat){
            ...
            case StatSelector.Oxygen:{
                defaultSurvivalStats.Oxygen = Mathf.Clamp(newMaxStatLevel,0,newMaxStatLevel);;
                if(Refill){currentSurvivalStats.Oxygen = Mathf.Clamp(newMaxStatLevel,0,newMaxStatLevel);}
                currentSurvivalStats.isGasping = (currentSurvivalStats.Oxygen<(defaultSurvivalStats.Oxygen/8));
...
public class SurvivalStats{
    public float Health = 250.0f, Hunger = 100.0f, Hydration = 100f, Oxygen = 100.0f;
    public bool hasLowHealth, isStarving, isDehydrated, isGasping;
}
public enum StatSelector{Health, Hunger, Hydration, Oxygen}
    ...
    public override void OnInspectorGUI(){
        ...
        currentSurvivalStats = SurvivalStatsTSO.FindProperty("currentSurvivalStats");
            ...
            if(survivalStatsFoldout){
                ...
                #region Oxygen Settings
                GUILayout.Label("<color=grey>Oxygen Settings</color>",labelSubHeaderStyle,GUILayout.ExpandWidth(true));
                EditorGUILayout.BeginVertical(BoxPanel);
                SerializedProperty statOX = defaultSurvivalStats.FindPropertyRelative("Oxygen"), currentStatOX = currentSurvivalStats.FindPropertyRelative("Oxygen");
                
                //preview bar
                Rect casingRectOX = EditorGUILayout.GetControlRect(), statRectOX = new Rect(casingRectOX.x+2, casingRectOX.y+2,Mathf.Clamp(((casingRectOX.width/statOX.floatValue)*currentStatOX.floatValue)-4, 0, casingRectOX.width), casingRectOX.height-4);
                EditorGUI.DrawRect(casingRectOX,statBackingColor);
                EditorGUI.DrawRect(statRectOX,new Color32(0,194,255,(byte)(GUI.enabled? 191:64)));
                
                EditorGUILayout.PropertyField(statOX,new GUIContent("Oxygen Points", "How much Oxygen does the controller start with?"));
                t.oxygenDepletionRate = EditorGUILayout.Slider(new GUIContent("Oxygen Depletion Per Tick","How much does oxygen deplete per tick?"), t.oxygenDepletionRate,0,5);
                GUI.enabled = false;
                EditorGUILayout.ToggleLeft(new GUIContent("Player is Gasping?"),currentSurvivalStats.FindPropertyRelative("isGasping").boolValue);
                GUI.enabled = t.enableSurvivalStats;
                EditorGUILayout.EndVertical();
                #endregion
            }
